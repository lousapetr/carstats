...
Currency exchange rates are fetched automatically from the Czech National
Bank's daily fixing (already CZK-denominated) the first time any non-CZK
currency is needed each day, then cached for the rest of the day — no cron
job or manual upkeep needed. See `app/currency/service.py::_ensure_rates_fresh`
and `app/currency/cnb_client.py`.
